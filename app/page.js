...
const HoverWord = memo(function HoverWord({ text, active, setHover }) {
  ...
  const handleClick = () => {
    ...
    if (text === "Branding") {
      if (reduce) {
        window.scrollBy({ top: 4000, behavior: "auto" });
      } else {
        const step = () => window.scrollBy({ top: 2000, behavior: "smooth" });
        step();
        setTimeout(step, 400);
        setTimeout(step, 800);
      }
  ...
  return (
    ...
    >
      {text}
      <span className="sr-only">{isActive ? " (selected)" : ""}</span>
    </button>
  );
});

/* ---------------- Sticky, animated intro (desktop) ---------------- */
function PinnedIntroAnimated({ hover, setHover }) {
  const ref = useRef(null);
  ...
  const heroOpacity = useSpring(heroOpacityT, SPRING);
  const heroY = useSpring(heroYT, SPRING);
  const aboutOpacity = useSpring(aboutOpacityT, SPRING);
  const aboutY = useSpring(aboutYT, SPRING);
  const servicesOpacity = useSpring(servicesOpacityT, SPRING);
  const servicesY = useSpring(servicesYT, SPRING);

  const aboutPE = useTransform(aboutOpacity, (v) => (v > 0.35 ? "auto" : "none"));
  const servicesPE = useTransform(servicesOpacity, (v) => (v > 0.35 ? "auto" : "none"));
    ...
    <section ref={ref} className="relative z-0 h-[600vh]" aria-label="Intro">
      <div className="sticky top-0 h-screen flex items-center justify-center overflow-visible [isolation:isolate]">
        ...
        <motion.div
          ...
          className="absolute inset-0 flex items-center justify-center will-change-transform transform-gpu"
        >
          <article className="max-w-[630px] px-3 text-center w-full flex flex-col gap-3">
            <p>
              Kane Fernandez is a 15-year-old web designer and developer who
              builds high-end sites for small businesses. Mentored by{" "}
              <a
                href="https://www.linkedin.com/in/ryandavidholmes"
                target="_blank"
                rel="noopener noreferrer"
...
/* ---------------- Static, accessible intro (mobile/reduced) ---------------- */
function PinnedIntroStatic({ hover, setHover }) {
  return (
    <section className="relative z-0">
      <div className="mx-auto max-w-[780px] px-4 py-24 space-y-24">
        <h1 className="text-center">Kane Fernandez</h1>

        <article className="max-w-[630px] mx-auto text-center">
          <p>
            Kane Fernandez is a 15-year-old web designer and developer who builds high-end sites for
            small businesses. Mentored by{" "}
            <a
              href="https://www.linkedin.com/in/ryandavidholmes"
              target="_blank"
              rel="noopener noreferrer"
...
export default function Page() {
  ...
  // Treat phones as reduced motion to dodge iOS sticky/viewport bugs
  const prefersReduced = useReducedMotion();
  const [isPhone, setIsPhone] = useState(false);
  useEffect(() => {
    const check = () => setIsPhone(window.matchMedia("(max-width: 767px)").matches);
    check();
    window.addEventListener("resize", check, { passive: true });
    return () => window.removeEventListener("resize", check);
  }, []);
  const useStaticIntro = prefersReduced || isPhone;
  ...
  // Preload opposite dataset on idle
  useEffect(() => {
    const other =
      hover === "Website"
        ? () => import("./props/branding-case-study-props")
    ...
    const id = ric(preload, { timeout: 1500 });
    return () => cic(id);
  }, [hover]);
  ...
  return (
    <MotionConfig reducedMotion="user" transition={SPRING}>
      <main className="relative h-fit overflow-visible">
        {!useStaticIntro && <DownArrows />}
        {useStaticIntro ? (
          <PinnedIntroStatic hover={hover} setHover={setHover} />
        ) : (
          <PinnedIntroAnimated hover={hover} setHover={setHover} />
        )}
